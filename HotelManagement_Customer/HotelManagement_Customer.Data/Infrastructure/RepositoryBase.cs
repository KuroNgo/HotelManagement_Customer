...
namespace HotelManagement_Customer.Data.Infrastructure
{
    public class RepositoryBase<T> : IRepository<T> where T : class
    {
        ...
        private HotelManagement_Customer_DbContext dataContext;
        ...
        protected HotelManagement_Customer_DbContext dbContext
        {
        ...
        {
            // Handle includes for association object if applicable
            if (includes != null && includes.Count() > 0)
            {
                var query = dataContext.Set<T>().Include(includes.First());
                foreach (var include in includes.Skip(1))
                    query = query.Include(include);
                return query.Where<T>(predicate).AsQueryable();
            }
